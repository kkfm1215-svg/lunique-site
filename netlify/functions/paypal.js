...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (action === 'capture-order') {
      ...
      // 캡처 응답에서는 custom_id가 purchase_unit이 아니라 captures[0]에 실려 온다.
      const cid = (cap && cap.custom_id) || pu.custom_id || '';
      if (cid && cid !== uid) return bad(403, 'order belongs to another account');
